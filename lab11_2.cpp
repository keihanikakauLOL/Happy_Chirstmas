...
int main(){	
    srand(time(0)); //(Completed) This line of code is not completed. Fill the correct input of srand(); 

	int playerScore, yugiScore, playerAction, yugiAction;
	int playerCards[3] = {drawCard(), drawCard(), 0};
	int yugiCards[3] = {drawCard(), drawCard(), 0}; //(Completed) This line of code is not completed. You need to initialize value of yugiCards[].
	...
	do{
		...
		cin >> playerAction;
	}while(playerAction != 1 && playerAction != 2); //(Completed) This line of code is not completed. You need to set the condition to do loop if user's input is not 1 or 2. 
	if(playerAction == 1){
